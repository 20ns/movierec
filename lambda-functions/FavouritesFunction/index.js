...
exports.handler = async (event) => {
  ...
  try {
    ...
    switch (event.httpMethod) {
      ...
      case 'POST':
        try {
          ...
          return createApiResponse(200, { message: "Added to favourites successfully" }, event);
        } catch (error) {
      ...
      case 'DELETE':
        try {
          ...
          await docClient.send(command);
          return createApiResponse(200, { message: "Removed from favourites successfully" }, event);
        } catch (error) {
    ...
    console.error("=== UNEXPECTED ERROR IN FAVOURITES FUNCTION ===", error);
