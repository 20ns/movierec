...
function AppContent() {
  ...
  // --- Render Logic ---
  // Modified to ensure it doesn't stay in loading state forever
  const showPageLoading = !initialAppLoadComplete || (isAuthenticated && preferencesLoading && refreshCycleRef.current < 3);
  
  // Debug loading state
  useEffect(() => {
    console.log("[App Debug] Loading state:", { 
      showPageLoading, 
      initialAppLoadComplete, 
      isAuthenticated, 
      preferencesLoading,
      refreshCycle: refreshCycleRef.current,
      userId: currentUser?.attributes?.sub
    });
  }, [showPageLoading, initialAppLoadComplete, isAuthenticated, preferencesLoading, currentUser]);
