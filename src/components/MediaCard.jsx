...
export const MediaCard = ({ result, onClick, promptLogin }) => {
  ...
  const safeResult = {
    ...
    poster_path: result?.poster_path || '',  // Fallback to empty string if poster_path is null/undefined
    media_type: result?.media_type || 'movie', // Default to 'movie' if media_type is missing
    overview: result?.overview || 'No overview available.', //Provide a fallback for overview
    vote_average: result?.vote_average || 0,
    release_date: result?.release_date || result?.first_air_date || '',
    first_air_date: result?.first_air_date || '',
    popularity: result?.popularity || 0,
    genre_ids: result?.genre_ids || [],
    score: result?.score || 0,  //added score for null value
    scoreReasons: result?.scoreReasons || [] //added scorereason for null value
  };
    ...
    }, [currentUser?.tokens?.idToken, safeResult.id]);
